function [J_of_t, dJdt, n_observations, new_known_map] = score_new(y, t, spacecraft_data)
...
relative_score_mapping = abs_score_mapping/abs_score_total * initial_score_mapping;
relative_score_features = abs_score_features/abs_score_total * initial_score_mapping;

single_score_features = score_map*relative_score_features;
single_score_mapping = relative_score_mapping/n_zones_unknown;
...
for j = 1:size(observations_run, 2)
    for i = 1:size(observations_run, 1)
        if observations_run(i, j) == 1
            ...
            observations_run(i, j) = 1;
            observations_score(i, j) = observations_run(i, j)*single_score_features(j);
            break
        end
    end
end
