...
%----PLANNING PHASE---------------------------------
    ...
    tree = MctsBeliefBased([plan_r_start; plan_v_start], plan_P_start, t_start, iterations, spacecraft_data_plan, initial_tree);
    [best_path, planned_actions_set, action_times_set, final_times_set] = find_best_path(tree);
    N = numel(final_times_set);
    N_set(i) = N;
    [planned_actions{i, 1:N}] = planned_actions_set{:};
    [action_times{i, 1:N}] = action_times_set{:};
    [final_times{i, 1:N}] = final_times_set{:};
    
    [ref_times_set, ref_trajectory_set] = compute_trajectory([plan_r_start; plan_v_start], action_times_set, final_times_set, planned_actions_set, model_dyn, options);
    [ref_trajectory{i, 1:N}] = ref_trajectory_set{:};
    [ref_times{i, 1:N}] = ref_times_set{:};

    all_trees{i} = tree;
    
    last_ref = ref_trajectory_set{N};
    plan_r_start = last_ref(end, 1:3)';
    plan_v_start = last_ref(end, 4:6)';

%----COASTING PHASE---------------------------------
    % 2) Introduce control error
    dvCorr2 = zeros(3,1);

    for j = 1:length(final_times_set)
        ...
        correction1{i, j} = dvCorr1;
        correction2{i, j} = dvCorr2;
        
        %Perform (perturbed) planned action
        err_magn = spacecraft_data.MC.pert_magn(spacecraft_data.MC.iter_man);
        err_align = spacecraft_data.MC.pert_align(spacecraft_data.MC.iter_man);
        err_theta = spacecraft_data.MC.pert_theta(spacecraft_data.MC.iter_man);
        spacecraft_data.MC.iter_man = spacecraft_data.MC.iter_man+1;
        [uu_real1, P_man1] = pertThrust(actCorr1, sigma_magn, sigma_align, err_magn, err_align, err_theta);
        ...
        [P_filtered, xx_filt, eta_f, flag, spacecraft_data_real] = navigationFilter([filter_r_start', filter_v_start' + actCorr1'], xx, real_P_start, tt, spacecraft_data_real);
        Pf = P_filtered(:, :, end);
    ...
    err_theta = spacecraft_data.MC.pert_theta(spacecraft_data.MC.iter_man);
    spacecraft_data.MC.iter_man = spacecraft_data.MC.iter_man+1;
    [uu_real, P_man] = pertThrust(dvCorr2, sigma_magn, sigma_align, err_magn, err_align, err_theta);
    ...
    [P_filtered, xx_filt_plan, eta_f, flag, spacecraft_data_real] = navigationFilter([filter_r_start', filter_v_start' + dvCorr2'], xx_real_plan, real_P_start, tt, spacecraft_data_real);
    
    real_trajectory{i,N+1} = xx_real_plan;
    ref_trajectory{i,N+1} = xx_ref_plan;
    filter_trajectory{i,N+1} = xx_filt_plan(:, 1:6);
    P_all{i, N+1} = P_filtered;
    ref_times{i, N+1} = tt;
    real_times{i, N+1} = tt;
    mapping_scores{i, N+1}    = zeros(size(tt));
    exploiting_scores{i, N+1} = zeros(size(tt));
    nav_scores{i, N+1}        = zeros(size(tt));
...
[real_trajectory_total,  tt_all] = concatTrajTimeCells(real_trajectory,  real_times,  N_set+1);
[ref_trajectory_total,   ~     ] = concatTrajTimeCells(ref_trajectory,   real_times,  N_set+1);
[filter_trajectory_total,~     ] = concatTrajTimeCells(filter_trajectory, real_times,  N_set+1);

mapping_score_total    = concatScalarCellsAligned(mapping_scores,   real_times, real_trajectory,  N_set+1);
exploiting_score_total = concatScalarCellsAligned(exploiting_scores,real_times, real_trajectory,  N_set+1);
nav_score_total        = concatScalarCellsAligned(nav_scores,       real_times, real_trajectory,  N_set+1);
