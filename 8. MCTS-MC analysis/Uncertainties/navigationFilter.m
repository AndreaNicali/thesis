function [P_filtered, xx_filtered, eta_f, flag, spacecraft_data] = navigationFilter(y0, y_truth, P0, tt, spacecraft_data)
...
if sigma_meas == 0
    perturbations = zeros(size(perturbations));
end
...
pert_meas = zeros(size(measurements.val));
    for i = 1:size(measurements.val, 2)
        elevation = measurements.val(1,i)+perturbations(index);
        index = index+1;
        azimuth = measurements.val(2,i)+perturbations(index);
        index = index+1;
        pert_meas(:, i) = [elevation;azimuth];
    end
measurements.val = pert_meas;

spacecraft_data.MC.iter_meas = index;
