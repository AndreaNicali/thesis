function [xx_filtered, P_filtered] = UKF_feokfe(x0, tt, measurements, P0, spacecraft_data, sigma_meas, Qk)
...
for i = 1:(length(tt)-1)
    ...
    [chi_aug] = sigma_points(x_minus, Pk_minus, alpha);
    chi_i_aug = chi_aug(:, 2:end);
    chi0_aug = chi_aug(:, 1);

    for j = 1:size(chi_aug, 2)

        sim_measurements = feature_measurements(chi_aug(:, j)', tt(i+1), spacecraft_data);
    ...
    Pxyk = Pxyk + Wic*(chi_i_aug(:, j) - x_minus)*(gamma_i(:, j) - y_minus)';

    end
    Peek = Peek + Rk;
        
    Peek = Peek + W0c*(gamma0 - y_minus)*(gamma0 - y_minus)';
    Pxyk = Pxyk + W0c*(chi0_aug - x_minus)*(gamma0 - y_minus)';
