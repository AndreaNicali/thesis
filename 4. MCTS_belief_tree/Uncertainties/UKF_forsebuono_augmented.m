function [xx_filtered, P_filtered] = UKF_forsebuono_augmented(x0, tt, measurements, P0, spacecraft_data, sigma_meas, Qk)
...
for i = 1:(length(tt)-1)
    ...
    if length(meas_azim) > 10
        meas_azim = meas_azim(1:10);
        meas_coal = meas_coal(1:10);
        feat_azim = feat_azim(1:10);
        feat_coal = feat_coal(1:10);
    end

    %GENERATE SIGMA POINTS
    x0 = [x0; zeros(size(Qk, 1), 1)];
    P0 = [P0, zeros(6, 6); zeros(6, 6), Qk];

    [sp] = sigma_points(x0, P0, alpha);

    chi = zeros(6, size(sp, 2));
    gamma = [];

    %GENERATE PROPAGATED POSITIONS AND MEASURMENTS
    for j = 1:size(sp, 2)

        [~ , prop] = ode78(@(t,x) model_rotating_dynamics(t, x, mass_eros, omega_body, C20, C22), [tt(i), tt(i+1)], sp(1:6, j), options);
        chi(:, j) = prop(end, :) + sp(7:12, j)';
