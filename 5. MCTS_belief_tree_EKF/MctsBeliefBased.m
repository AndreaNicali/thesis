    ...
    while new_node == 0
        ...
        if depth == 0 %Select root
            ...
            if tree{id_parent}.expanded == 0 %Node is expanded i.e. actions from that node are computed
                ...
                %Add to all the Belief "brothers" node the same actions
                %(reduce computations)
        ...
        %If an i-th node can be added, select the best untried action to add to the node
        if length(parent_node.children)+1 < N_children_max
            ...
            [P_filtered, xx_filt] = navigationFilter(state', xx, P, tt, parent_data);
            Pf = P_filtered(:, :, end);
